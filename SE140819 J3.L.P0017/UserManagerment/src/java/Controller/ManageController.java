...
public class ManageController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String statuss = request.getParameter("txtStatus");
        boolean status = false;
        
        
        try {
            if("true".equals(statuss)){
                status = true;
            }
            UserDTO user = new UserDTO(Integer.parseInt(id), name, password, email, 
                    phone, photo, status, Integer.parseInt(rank), Integer.parseInt(role), Integer.parseInt(proID));
