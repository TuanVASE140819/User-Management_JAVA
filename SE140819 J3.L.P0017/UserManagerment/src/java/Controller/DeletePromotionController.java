...
public class DeletePromotionController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String url = "promotionDetail.jsp";
        String proID = request.getParameter("txtProID");
        String userID = request.getParameter("txtUserID");
        ProDetailDTO tmp = null;
        
        // get listDetail and romove
        
        try{
           HttpSession session = request.getSession(false);
            if (session!=null) {
                List<ProDetailDTO> list=(List<ProDetailDTO>) session.getAttribute("listDetail");
                for (ProDetailDTO d : list) {
                    if (d.getUser().getUserID()==Integer.parseInt(userID)&&d.getProID()==Integer.parseInt(proID)) {
                        tmp= d;
                    }
                }
                if (tmp!=null) {
